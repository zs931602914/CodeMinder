...
- **智能粘贴** — Ctrl+V / 右键粘贴自动识别图片与文本，截图可直接传给 Claude Code
...
CodeMinder 的粘贴（`Ctrl+V` 或右键菜单 → 粘贴）会自动判断剪贴板内容类型，按以下优先级处理：

| 优先级 | 剪贴板内容 | 处理方式 |
|--------|-----------|---------|
| 1 | 图片像素数据（截图、画板复制等） | 保存为临时 PNG 文件，将**文件路径**粘贴到终端 |
| 2 | 图片文件（从文件管理器复制的图片） | 直接将**原文件路径**粘贴到终端 |
| 3 | 其他文件（非图片） | 将文件路径作为文本粘贴 |
| 4 | 文本 | 直接粘贴文本 |
| 5 | 空剪贴板 | 不执行任何操作 |

> **说明**：终端无法直接渲染图片，因此粘贴图片时实际输入的是图片的本地路径。在 Claude Code 等支持图片引用的 CLI 中，该路径会被识别为图片输入。

**支持的图片格式**：`png`、`jpg`、`jpeg`、`gif`、`webp`、`bmp`

**典型用法**：

- `Win + Shift + S` 截图后直接 `Ctrl+V`，图片路径自动填入 Claude Code，无需手动保存文件
- 在文件管理器中复制图片文件后粘贴，直接传入文件路径
